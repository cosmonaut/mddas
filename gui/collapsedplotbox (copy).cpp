...
CollPlotBox::CollPlotBox(QMap<QString, QVariant> settings, QWidget *parent) : QWidget(parent) {
...
void CollPlotBox::setPrefs(void) {
    _specPlotConf->exec();
}
...
void CollPlotBox::configBoxes(void) {
    QMap<QString, uint> m;

    m = _specPlotConf->getParams();

    _plot->setBox(m["b1x"], (m["b1x"] + m["b1w"]), m["b1y"], (m["b1y"] + m["b1h"]));
    _plot->setRebinFactor(m["b1r"]);
...
/* Signal for spectroscatterplot to set box position */
    /* reconfig plot
       update settings
       update specploftconfig settings
       emit settings changed */
//    if (box == 1) {
    _plot->setBox(x,
...
QMap<QString, QVariant> CollPlotBox::getSettings(void) {
    QMap<QString, QVariant> m;
    m = (*_settings);
    return(m);
}

CollPlotConfig::SpecPlotConfig(QMap<QString, QVariant> settings, QWidget *parent) : QDialog(parent){
